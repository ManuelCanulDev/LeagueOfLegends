    ...
    <section class="page-section portfolio mt-5" id="portfolio">
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-md-6">
                    <h4>Add Stat of <?php echo $champion['name']; ?></h4>
                    <form method="post" id="upload_form" enctype="multipart/form-data">
                        ...
                        <br>
                        <div class="form-group">
                            <label for="modifier_per_level">Modifier per level</label>
                            <input type="text" name="modifier_per_level" id="modifier_per_level" class="form-control" placeholder="modifier_per_level">
                        </div>
                        <br>
    ...
    <script>
        $(document).ready(function () {
            $('#upload_form').on('submit', function (e) {
                e.preventDefault();
                if ($('#name').val() == '' || $('#value').val() == '' || $('#modifier_per_level').val() == '') {
                    alert("Faltan parametros");
                } else {
                    $.ajax({
                        url: "<?php echo base_url(); ?>api/v1/stats/add",
                        ...
                        success: function (data) {
                            if (data['message'] == 'OK') {
                                alert('Estadistica de <?php echo $champion['name']; ?> Agregad@');
                                if (confirm('Desea agregar otra estadistica para <?php echo $champion['name']; ?>?')) {
                                    // Save it!
                                    window.location.href = "<?php echo base_url('add-stats/'.$id_champion); ?>";
                                } else {
                                    // Do nothing!
